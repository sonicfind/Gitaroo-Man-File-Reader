...
bool CHCType::loadSingle(string filename)
{
	bool val = false;
	try
	{
...
bool CHCType::loadMulti()
{
	do
	{
		...
		if (m_files.size() > 1)
		{
			...
			for (size_t i = 1; i < g_filetypes.size(); ++i)
			{
		...
		case GlobalFunctions::ResultType::Success:
			...
			else
			{
				...
				while (m_files.size())
				{
					try
					{
						CHC_Main chc(m_files.front());
						switch (choice)
						{
						case 'e':
							if (chc.menu(m_files.size()))
							{
								--g_global;
								return false;
							}
...
bool CHC_Main::menu(size_t fileCount)
{
	...
	MenuPrompt:
		...
		case GlobalFunctions::ResultType::Help:
		{
			...
			if (m_song.m_unorganized > 0 || !m_song.m_optimized)
			{
				printf("%sF - Fix & organize %s.CHC where necessary (may not get all present bugs)\n", g_global.tabs.c_str(), m_song.m_shortname.c_str());
				printf("%sMany CHC files provided from the base game are very prone to a multitude of glitches or contain sizable mistakes.\n", g_global.tabs.c_str());
				printf("%sFor example: Stage 8 having two guard marks being only 2 samples away from eachother, or Stage 3 having a broken attack phrase note.\n", g_global.tabs.c_str());
...
{
	GlobalFunctions::banner(" " + m_song.m_shortname + ".CHC Save Prompt ");
	string ext = "_T";
	...
	do
	{
		...
		case GlobalFunctions::ResultType::Success:
			...
			case 's':
				...
				m_song.create(m_song.m_filename);
				m_song.m_saved = 2;
				g_global.quit = true;
				break;
			case 'a':
...
{
	GlobalFunctions::banner(" Writing " + m_song.m_shortname + "_CHC.txt ");
	FILE* outTXT, * outSimpleTXT;
	fopen_s(&outTXT, (m_song.m_filename.substr(0, m_song.m_filename.length() - 4) + "_CHC.txt").c_str(), "w");
	fopen_s(&outSimpleTXT, (m_song.m_filename.substr(0, m_song.m_filename.length() - 4) + "_CHC_SIMPLIFIED.txt").c_str(), "w");
	try
	{
		fprintf(outTXT, "Header: %s", m_song.m_header);
		GlobalFunctions::dualvfprintf_s(outTXT, outSimpleTXT, "IMC: ");
		m_song.m_imc[0] ? GlobalFunctions::dualvfprintf_s(outTXT, outSimpleTXT, m_song.m_imc)		//IMC
		...
		if (m_song.m_imc[0])
		{
			for (size_t index = 0; index < 8; index++)
			{
				...
				GlobalFunctions::dualvfprintf_s(outTXT, outSimpleTXT, "\t\t       Volume: %lu (%g%%)\n", m_song.m_audio[index].volume, m_song.m_audio[index].volume * 100.0 / 32767);
				...
				default: GlobalFunctions::dualvfprintf_s(outTXT, outSimpleTXT, "\t\t\t  Pan: %g%% Left | %g%% Right (%lu)\n", 100 - (m_song.m_audio[index].pan * 100.0 / 32767),
					m_song.m_audio[index].pan * 100.0 / 32767, m_song.m_audio[index].pan);
				}
			}
		}
		else
		...
		fprintf(outTXT, "\t   # of Cues: %zu\n", m_song.m_sections.size());
		fputs("\t    SSQ Cues:\n", outTXT);
		for (size_t cueIndex = 0; cueIndex < m_song.m_sections.size(); cueIndex++)	//Cues
		{
			SongSection& section = m_song.m_sections[cueIndex];
		...
		for (size_t sectIndex = 0; sectIndex < m_song.m_sections.size(); sectIndex++) //SongSections
		{
			...
			for (size_t condIndex = 0; condIndex < section.m_conditions.size(); condIndex++)
			{
				...
				{
					if ((size_t)cond.m_trueEffect < m_song.m_sections.size()) GlobalFunctions::dualvfprintf_s(outTXT, outSimpleTXT, "\t\t\t\t\t True Effect: Move to Section %s.\n", m_song.m_sections[cond.m_trueEffect].m_name);
				...
				if (cond.m_type != 0)
				{
					...
					{
						if ((size_t)cond.m_falseEffect < m_song.m_sections.size()) GlobalFunctions::dualvfprintf_s(outTXT, outSimpleTXT, "\t\t\t\t\tFalse Effect: Move to Section %s.\n", m_song.m_sections[cond.m_falseEffect].m_name);
		...
		for (size_t player = 0; player < 4; player++)
		{
			...
			for (size_t section = 0; section < 5; section++)
			{
				...
				GlobalFunctions::dualvfprintf_s(outTXT, outSimpleTXT, "%015g%% ||%020g%% ||%020g%% ||%017g%% ||%018g%% ||%017g%% ||%018g%% ||%018g%% ||\n",
					m_song.m_energyDamageFactors[player][section].initialEnergy * 100.0,
					m_song.m_energyDamageFactors[player][section].chargeInitial * 100.0,
					m_song.m_energyDamageFactors[player][section].attackInitial * 100.0,
					m_song.m_energyDamageFactors[player][section].guardEnergy * 100.0,
					m_song.m_energyDamageFactors[player][section].attackMiss * 100.0,
					m_song.m_energyDamageFactors[player][section].guardMiss * 100.0,
					m_song.m_energyDamageFactors[player][section].chargeRelease * 100.0,
					m_song.m_energyDamageFactors[player][section].attackRelease * 100.0);
			}
			GlobalFunctions::dualvfprintf_s(outTXT, outSimpleTXT, "\n");
		}
	}
...
bool CHC_Main::makeTAS()
{
	TAS tas;
	return tas.build(m_song);
}

bool CHC_Main::exportChart()
{
	GlobalFunctions::banner(" " + m_song.m_shortname + " - .CHART creation ");
	if (m_song.m_unorganized != 0)
	{
		do
		{
	...
	CH_Exporter exporter(m_song);
	return exporter.exportChart();
}

bool CHC_Main::importChart()
{
	GlobalFunctions::banner(" " + m_song.m_shortname + " - .CHART Note Import/Replacement ");
	if (m_song.m_unorganized != 0)
	{
		do
		{
	...
	CH_Importer importer(m_song);
	if (importer.importChart())
	{
		m_song = importer.getSong();
		return true;
	}
	else
		return false;
...
{
	GlobalFunctions::banner(" " + m_song.m_shortname + ".CHC - Color Sheet Creation ");
	bool multiplayer = toupper(m_song.m_shortname.back()) == 'M';
	if (!multiplayer)
	{
		do
		{
	...
	g_global.quit = false;
	string filename = m_song.m_filename.substr(0, m_song.m_filename.length() - 4) + "_COLORDEF";
	string filename2 = m_song.m_filename.substr(0, m_song.m_filename.length() - 4) + "_COLORDEF_FRAGS";
