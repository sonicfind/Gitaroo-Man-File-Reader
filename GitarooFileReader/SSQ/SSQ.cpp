...
SSQ::SSQ(std::string filename, bool loadXGM) : m_filename(filename + ".SSQ"), m_saved(2)
{
	...
	GlobalFunctions::banner(" Loading " + m_shortname + ".SSQ ");
	union {
		char c[4];
		unsigned ui;
	} u;
	...
	fread(&numXG, 4, 1, inFile);

	unsigned long numIMX = 0;
	fread(&numIMX, 4, 1, inFile);

	for (unsigned long i = 0; i < numIMX; ++i)
		m_IMXentries.emplace_back(inFile);

	fread(&numXG, 4, 1, inFile);

	for (unsigned long i = 0; i < numXG; ++i)
		m_XGentries.emplace_back(inFile);
	...
	for (unsigned long i = 0; i < numXG; ++i)
	{
		...
		case ModelType::Normal:
			m_modelSetups.push_back(std::make_shared<ModelSetup>(inFile, m_XGentries[i].m_name));
			break;
		case ModelType::Player1:
		case ModelType::Player2:
		case ModelType::DuetPlayer:
			m_modelSetups.push_back(std::make_shared<PlayerModelSetup>(inFile, m_XGentries[i].m_name));
			break;
		case ModelType::Player1AttDef:
		case ModelType::Player2AttDef:
		case ModelType::DuetPlayerAttDef:
		case ModelType::DuetComboAttack:
			m_modelSetups.push_back(std::make_shared<AttDefModelSetup>(inFile, m_XGentries[i].m_name));
			break;
		case ModelType::Snake:
			m_modelSetups.push_back(std::make_shared<SnakeModelSetup>(inFile, m_XGentries[i].m_name));
		}
		
	}

	m_camera.read(inFile);

	m_sprites.read(inFile);

	if (m_headerVersion >= 0x1100)
	{
		unsigned long numTex;
		fread(&numTex, 4, 1, inFile);
		for (unsigned long i = 0; i < numTex; ++i)
			m_texAnimations.emplace_back(inFile);
	}
...
void SSQ::create(std::string filename)
{
	{
		size_t pos = filename.find_last_of('\\');
		GlobalFunctions::banner(" Saving " + filename.substr(pos != std::string::npos ? pos + 1 : 0) + ' ');
	}
	FILE* outFile;
	fopen_s(&outFile, filename.c_str(), "wb");
	fprintf(outFile, "GMSX");
	...
	for (auto& model : m_modelSetups)
		model->create(outFile);

	m_camera.create(outFile);

	m_sprites.create(outFile);
