...
void xgNormalInterpolator::create(FILE* outFile, bool full) const
{
...
void xgNormalInterpolator::replaceVertexData() const
{
	const ListType<glm::vec3> list = interpolate();
	for (size_t i = 0; i < list.m_values.size(); ++i)
	{
		glBufferSubData(GL_ARRAY_BUFFER, sizeof(Vertex) * i + sizeof(glm::vec4), sizeof(glm::vec3), glm::value_ptr(list.m_values[i]));
		/*size_t offset = 0;
