...
bool GitarooViewer::Dag::load(XGM* xgm, XG_Data::DagBase& dagBase)
{
	...
	{
		m_mesh = new DagMesh;
		m_transparency = m_mesh->load(xgm, (xgDagMesh*)dagBase.m_base.get());
		return m_transparency;
	}
}
...
{
	m_transform = (xgDagTransform*)dagBase.m_base.get();
...
bool GitarooViewer::DagMesh::load(XGM* xgm, xgDagMesh* mesh)
{
	...
	if (newGeometry)
	{
		...
		m_vertices = std::make_shared<float*>(new float[m_size]);
		std::copy(geo->m_vertices, geo->m_vertices + m_size, *m_vertices);
		glBufferData(GL_ARRAY_BUFFER, 4 * m_size, *m_vertices, GL_STATIC_DRAW);
...
void GitarooViewer::DagMesh::draw(const float time, glm::mat4 model, const bool showNormals)
{
	...
	for (auto& mat : m_materials)
	{
		...
		glBindBuffer(GL_ARRAY_BUFFER, m_VBO);
		glBufferSubData(GL_ARRAY_BUFFER, 0, 4 * m_size, *m_vertices);
		glBindVertexArray(m_VAO);
