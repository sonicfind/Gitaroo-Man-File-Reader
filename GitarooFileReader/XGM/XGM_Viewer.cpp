...
void XGM_Viewer::mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
	g_camera.turnCamera(window, xpos, ypos);
}

void XGM_Viewer::scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
{
	g_camera.zoom(window, xoffset, yoffset);
}

int XGM_Viewer::viewXG(XGM* xgmObject, const std::vector<size_t>& xgIndices, const float tempo)
{
	changeBPM(tempo);
	g_camera.reset();
	...
	while (!glfwWindowShouldClose(m_window))
	{
		...
		g_camera.moveCamera(m_window, m_currentTime - m_previousTime);
...
bool GitarooViewer::DagMesh::load(XGM* xgm, xgDagMesh* mesh, Timeline& timeline, size_t transformIndex)
{
	...
	if (newGeometry)
	{
		...
		m_vertices = std::shared_ptr<Vertex*>(new Vertex * (new Vertex[m_numVerts]), [&](auto p) { delete[m_numVerts] * p; });
		...
		glBufferData(GL_ARRAY_BUFFER, m_numVerts * sizeof(Vertex), *m_vertices, GL_STATIC_DRAW);
		...
		if (geo->m_inputEnvelopes.size())
		{
			...
			m_boneVertices = std::shared_ptr<BoneVertex*>(new BoneVertex * (new BoneVertex[m_numVerts]), [&](auto p) { delete[m_numVerts] *p; });
			...
			for (unsigned long index = 0; index < m_numVerts; ++index)
				memcpy(*m_boneVertices + index, verts + index, sizeof(Vertex));
			...
			glBufferData(GL_ARRAY_BUFFER, m_numVerts * sizeof(BoneVertex), *m_boneVertices, GL_STATIC_DRAW);
		}
				...
				geo->m_inputTexCoordInterpolator.isValid())
		{
			...
			shape.m_animated = new ShapeVertex[m_numVerts];
			m_shapeVertices = std::shared_ptr<ShapeVertex*>(new ShapeVertex*(shape.m_animated));
			glBufferData(GL_ARRAY_BUFFER, m_numVerts * sizeof(ShapeVertex), *m_shapeVertices, GL_DYNAMIC_DRAW);
...
void GitarooViewer::Model::draw(const float time, glm::mat4 base, const bool showNormals)
{
	for (auto& dag : m_meshes)
	{
		...
		else
		{
			...
			{
				glBufferSubData(GL_ARRAY_BUFFER, 0, dag->m_numVerts * sizeof(ShapeVertex), *dag->m_shapeVertices);
